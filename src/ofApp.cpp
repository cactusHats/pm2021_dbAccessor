...
int ofApp::readSettingFile() {
	...
	//-- Load setting file --
	ifstream infile("./data/setting.conf"); //.exeから起動するとき
...
int ofApp::readTable(vector<vector<string>>& tableData_) {
	...
	while (result->next()) {
		...
		tmp.push_back(result->getString(int(DATA_SET::QR_ID) + 1)); //QR_CODE_ID
		...
		/*
		printf("Reading from table = (JB=%s, QR=%s, PC=%s, TYPE=%s, LUNCH=%s, UPDATE_AT=%s)\n", 
			tmp[int(DATA_SET::JB_ID)].c_str(),
			tmp[int(DATA_SET::QR_ID)].c_str(),
...
int ofApp::sendOscPacket(vector<vector<string>> sendData, bool testMode) {
	...
	if (testMode == false) {
		...
		for (int i = 0; i < sendData.size(); i++) {
			...
			//m.addIntArg(stoi(sendData[i].at(int(DATA_SET::QR_ID)))); //QR_CODE_ID
			...
			snd.sendMessage(m);
			printf("%s %s send data = ID=%s, QR=%s, PC=%s, TYPE=%s, LAUNCHED=%s, UPDATED_AT=%s\n",
				getCurrentDate().c_str(),
				getCurrentTime().c_str(),
				sendData[i].at(int(DATA_SET::JB_ID)).c_str(),
				sendData[i].at(int(DATA_SET::QR_ID)).c_str(),
...
int ofApp::createTable() {
	...
	//-- Create query --
	string query = "CREATE TABLE fireWorksTable (JOB_ID INT(11) not null AUTO_INCREMENT PRIMARY KEY, QR_CODE_ID INT(11) not null, PC_ID INT(11) not null, TYPE_ID INT(11) not null, LAUNCHED BOOLEAN not null, UPDATED_AT TIMESTAMP not null) engine = innodb default charset = utf8";
	//cout << "QUERY= " << query << endl;
...
int ofApp::insertData() {
	...
	//-- Create query --
	string query = "insert INTO fireworkstable (QR_CODE_ID, PC_ID, TYPE_ID, LAUNCHED) VALUES (3,0," + to_string(type) + ",0);";
	//cout << "QUERY= " << query << endl;
